...
#print "$sample,$chr,$readlen,$file\n";
	my $size = (-s $file) || 0;
	if ($readlen >= $maxRL and $size > $maxSize) {	# the longer reads can stands for the shorter ones
		$maxSize = $size;
		$maxFile = $file;
		$maxRL = $readlen;
		$maxChr = $chr;
...
my $sh="$bin -i $maxFile -d $ref/$maxChr -o /dev/null -F 1 -M $out.matrix0 -L $maxRL 2>$out.log";
