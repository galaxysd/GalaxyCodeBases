/*
 * shsdual - old shs dual digest code
 *
 * @(#) $Revision: 13.2 $
 * @(#) $Id: shsdual.c,v 13.2 2006/08/14 10:09:23 chongo Exp $
 * @(#) $Source: /usr/local/src/cmd/hash/RCS/shsdual.c,v $
 *
 **************************************************************************
...
/* static declarations */
static void multiData(BYTE*, UINT, BYTE*, UINT, UINT, SHS_INFO*);
static void multiStream(BYTE*, UINT, FILE*, UINT, SHS_INFO*);
static void multiFile(BYTE*, UINT, char*, UINT, SHS_INFO*);
static void multiOutput(char*, int, int, UINT, SHS_INFO*);
 ...
 */
static void
multiData(BYTE *pre_str, UINT pre_len, BYTE *inString, UINT in_len,
    UINT cnt, SHS_INFO *digs)
{
     ...
     */
    for (i=0; i < cnt; ++i) {
	shsUpdate(digs+i, bufs[i], buflen[i]);
    }
 ...
 */
static void
multiStream(BYTE *pre_str, UINT pre_len, FILE *stream, UINT cnt, SHS_INFO *digs)
{
    ...
    for (i=0; i < cnt; ++i) {
	bufs[i] = (BYTE *)malloc(SHS_BLOCKSIZE);
	if (bufs[i] == NULL) {
	...
	for (p=pre_str, pbeyond=pre_str+pre_len; p < pbeyond; ++p, ++indx) {
	    *(n[(indx = ((indx >= cnt) ? 0 : indx))]++) = *p;
	    if (++buflen[indx] >= SHS_BLOCKSIZE) {
		shsUpdate(digs+indx, bufs[indx], SHS_BLOCKSIZE);
		buflen[indx] = 0;
		n[indx] = bufs[indx];
	    }
	}
 ...
 */
static void
multiFile(BYTE *pre_str, UINT pre_len, char *filename, UINT cnt, SHS_INFO *digs)
{
	...
	for (i=0; i < cnt; ++i) {
	    if (digs[i].datalen > 0) {
		shsUpdate(digs+i, (BYTE *)shs_zero,
			   SHS_CHUNKSIZE - digs[i].datalen);
	    }
	}
    }
...
multiOutput(str, quot, nospace, cnt, digs)
    ...
    SHS_INFO *digs;	/* array of digests, cnt elements long */
{
    UINT i;
     ...
     * print the digests
     */
    shsPrint(c_flag, 0, digs);
    for (i=1; i < cnt-1; ++i) {
	if (nospace == 0) {
	    putchar(' ');
	    shsPrint(c_flag, 0, digs+i);
	} else {
	    shsPrint(0, 0, digs+i);
	}
    }
	...
	if (nospace == 0) {
	    putchar(' ');
	    shsPrint(c_flag, i_flag, digs+cnt-1);
	} else {
	    shsPrint(0, i_flag, digs+cnt-1);
	}
    }
    if (str && !q_flag) {
	if (quot) {
...
multiTest(void)
{
    ...
    SHS_INFO digs[2];		/* even byte digest */
...
/*
 * multiMain - main driver of shs dual routines
 *
 * given:
 *	c		arg count left after getopt
    ...
    char *data_str, int nospace, UINT cnt)
{
     ...
     */
    digs = (SHS_INFO *)malloc(sizeof(SHS_INFO)*cnt);
    if (digs == NULL) {
	fprintf(stderr, "%s: bad malloc #1\n", program);
	exit(60);
    }
