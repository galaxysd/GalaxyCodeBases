 ...
 *
 * @(#) $Revision: 13.3 $
 * @(#) $Id: md5.c,v 13.3 2006/08/14 11:25:24 chongo Exp $
 * @(#) $Source: /usr/local/src/cmd/hash/RCS/md5.c,v $
 *
 ...
 */
void
MD5Update(MD5_CTX *md5Ctx, BYTE *inBuf, UINT count)
{
    ULONG datalen = md5Ctx->datalen;
#if BYTE_ORDER == BIG_ENDIAN
    ULONG in[MD5_CHUNKWORDS];	/* aligned buffer */
    ULONG tmp;
    unsigned int cnt;
#endif
 ...
 */
void
MD5fullUpdate(MD5_CTX *md5Ctx, BYTE *inBuf, UINT count)
{
#if BYTE_ORDER == BIG_ENDIAN
    ULONG in[MD5_CHUNKWORDS];	/* aligned buffer */
    ULONG tmp;
 ...
 */
void
MD5Transform(ULONG *buf, ULONG *in)
{
