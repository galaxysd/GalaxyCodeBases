...
FileListManager::~FileListManager()
{
	...
	for (vector<ofstream*>::iterator iter = m_cns_file_vec.begin();
	...
	for (vector<ifstream*>::iterator iter = m_soap_file_vec.begin();
...
int FileListManager::openAli(const string& listfilename,const char* mode)
{
	...
	{
		return openSamFile(listfilename,mode);            //open sam/bam file
	}
	else 
	{
		return openSoapFile(listfilename);                   //open soap file    
	}
}
 ...
 */
int FileListManager::readWin(vector<Readwin>& readwin_vec)
{
	int ret = 0;
	...
	else if (m_sam_file_vec.size() == readwin_vec.size())
	{
		for (int i = 0; i < readwin_vec.size(); ++i)
		{
			// read records from sam/bam file to the win.
 ...
 */
ifstream* FileListManager::getSoapFile(const int index)
{
 ...
 */
int FileListManager::readWinFromSoap(Readwin& readwin, ifstream& soapfile)
{
 ...
 */
ofstream* FileListManager::getCnsFile(const int index)
{
...
int FileListManager::openMatrixFile(const std::string matrix_list, std::ios_base::open_mode mode, const std::string alignment_list)
{
	...
	{
		string filepath;
		m_base_file.open(matrix_list.c_str());              //open soap filelist
		if (!m_base_file) 
		{
	...
	else if (alignment_list != "")
	{
		...
		m_base_file.open(alignment_list.c_str());  //open filelist
