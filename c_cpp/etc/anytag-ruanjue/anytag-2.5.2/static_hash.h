...
static inline size_t rank_##hash_type(hash_type *set, hash_key_type *ptr){ return rank_bitvec(set->flags, ptr - set->array); }	\
static inline size_t dump_##hash_type(hash_type *set, FILE *out){              \
	...
	n += ffwrite(set->array, sizeof(hash_key_type), set->size, out);	\
