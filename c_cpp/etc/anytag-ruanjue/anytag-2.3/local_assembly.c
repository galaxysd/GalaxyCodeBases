...
void init2_lgraph(LGraph *g, ATOptions *opt){
	...
	g->aux         = sr_init_aux(1, opt->min_ol[1], opt->max_mm[1], opt->min_sm[1], opt->allow_gap[1], 0, opt->max_hits[1]);
	g->long_aux    = sr_init_aux(3, opt->min_ol[2], opt->max_mm[2], opt->min_sm[2], opt->allow_gap[2] & 0x01, 0, opt->max_hits[2]);
	g->nodes       = init_nodev(16);
	g->m_node      = 0;
	g->seqnodes    = init_snv(16);
	g->m_seqnode   = 0;
...
void align_lgraph(LGraph *g){
	...
	Node *n1, *n2;
	Edge *e;
	uint32_t i, j, beg, offb, offe, rid1, rid2, n_ol;
	g->n_sr = g->nodes->size;
	...
	for(i=0;i<g->sdb->n_rd;i++){
		n1 = ref_nodev(g->nodes, i);
		//if(n1->edges[0]->size && n1->edges[1]->size && g->nodes->size > 1000 && i >= 2) continue;
		for(j=0;j<n1->edges[0]->size;j++) sr_block_aux(g->aux, ref_edgev(n1->edges[0], j)->nid);
		for(j=0;j<n1->edges[1]->size;j++) sr_block_aux(g->aux, ref_edgev(n1->edges[1], j)->nid);
		beg = g->aux->hits->size;
		...
		for(j=beg;j<count_sr_hitv(g->aux->hits);j++){
			...
			e = next_ref_edgev(n1->edges[0]); e->nid = rid2; e->n_ol = n_ol; e->off = offb; e->hit_idx = j; e->mut_idx = n2->edges[1]->size; e->closed = 0;
			e = next_ref_edgev(n2->edges[1]); e->nid = rid1; e->n_ol = n_ol; e->off = offe; e->hit_idx = j; e->mut_idx = n1->edges[0]->size - 1; e->closed = 0;
		}
	}
}
...
void align2_lgraph(LGraph *g){
	...
	sr_ready_sdb(g->sdb);
	sr_clear_aux(g->aux);
	for(i=0;i<g->sdb->n_idx;i++) sr_index2_sdb(g->sdb, i);
	sr_align2_sdb(g->sdb, g->aux);
	for(i=0;i<g->aux->hits->size;i++){
...
uint32_t align_sr2lgraph(LGraph *g, SR_SeqDB *sdb){
	...
	for(i=0;i<g->seqnodes->size;i++){
		...
		g->aux->strand_type = 3;
		sr_align_long_sdb(sdb, g->aux, i, g->bits->buffer, 0, sn->len);
		g->aux->strand_type = 1;
		ret += g->aux->hits->size;
