...
void push_lgraph(LGraph *g, uint32_t rid, char *seq, uint8_t seqlen, uint32_t seqdir){
	Node *node;
		...
		if(i == count_u32list(g->aux_ords) || ref_nodev(g->nodes, get_u32list(g->aux_ords, i))->asm_off != ref_nodev(g->nodes, get_u32list(g->aux_ords, x))->asm_off){
			if(i - x > 1){
				for(j=x;j<i;j++){
					...
					if((nid1 & 0x01) == 0) continue;
					for(k=j+1;k<i;k++){
						nid2 = get_u32list(g->aux_ords, k);
						if((nid2 & 0x01) == 0) continue;
						if(ref_nodev(g->nodes, nid1 & 0xFFFFFFFEU)->asm_off != ref_nodev(g->nodes, nid2 & 0xFFFFFFFEU)->asm_off) continue;
						if(strcmp(get_strv(g->srs, nid1)->string, get_strv(g->srs, nid2)->string) != 0) continue;
						if(strcmp(get_strv(g->srs, nid1 & 0xFFFFFFFEU)->string, get_strv(g->srs, nid2 & 0xFFFFFFFEU)->string) != 0) continue;
						put_i32hash(hash, nid2);
						put_i32hash(hash, nid2 & 0xFFFFFFFEU);
					}
				}
			}
			x = i;
...
void output_lgraph(LGraph *g, FILE *cnsf, FILE *samf, FILE *msaf){
	...
	if(msaf){
		...
		for(i=0;i<count_u32list(g->ords);i++){
			...
			fprintf(msaf, "%010u %s\t%s\n", ref_nodev(g->nodes, get_u32list(g->ords, i))->rid, get_strv(g->srs, get_u32list(g->ords, i))->string, ac);
		}
	}
}
