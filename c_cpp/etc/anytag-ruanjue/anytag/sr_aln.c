...
SR_AlnAux* sr_init_aux(uint32_t strand, uint32_t max_ol, uint32_t min_ol, uint32_t max_mm, float min_sm, int gap, int filter_dup, uint32_t max_hits){
	SR_AlnAux *aux;
...
void sr_set_overlap_aux(SR_AlnAux *aux, uint32_t max_ol, uint32_t min_ol){
	aux->max_overlap = max_ol;
	aux->min_overlap = min_ol;
}
...
size_t sr_dump_sdb(SR_SeqDB *sdb, FILE *out){
	...
	sdb->rd_seqs->size = sdb->rd_seqs->cap;
	size += dump_u64list(sdb->rd_seqs, out);
	sdb->rd_seqs->size = 0;
	size += dump_u16list(sdb->rd_lens, out);
	size += dump_u64list(sdb->rd_offs, out);
...
SR_SeqDB* sr_load_sdb(FILE *inp){
	...
	if((sdb->rd_seqs = load_u64list(inp)) == NULL) return NULL;
	if((sdb->rd_lens = load_u16list(inp)) == NULL) return NULL;
	if((sdb->rd_offs = load_u64list(inp)) == NULL) return NULL;
...
void sr_index_sdb(SR_SeqDB *sdb, uint32_t n_idx){
	...
	uint64_t offset;
	int exists;
	if(sdb->idx_pats[n_idx][2] > 1) return;
	if(sdb->indexs[n_idx]){
		free_sr_prm_hash(sdb->indexs[n_idx]);
		sdb->indexs[n_idx] = NULL;
	}
	hash = init2_sr_prmhash(1024, sdb->hash_factor);
	P.rid = 0;
	...
	sdb->indexs[n_idx] = sr_prmhash2sr_prm_hash(hash);
...
static inline uint64_t sr_stdaln_lh3(AlnCigar* cigars, AlnParam *ap, uint64_t *seqs1, uint64_t *seqs2, uint64_t off1, uint64_t off2, uint32_t dir1, uint32_t dir2, uint32_t len1, uint32_t len2){
	...
	aln = aln_stdaln_aux(seq1, seq2, ap, 0, 0, len1, len2);
	aln->start1 --;
	aln->start2 --;
	mm = mn = 0;
	n_cigar = 0;
...
uint32_t _query_index_and_add_ptrv(SR_SeqDB *sdb, SR_AlnAux *aux, uint32_t idx, SR_Primer PRM, uint32_t off1, uint32_t off2, uint32_t dir1, uint32_t dir2, uint32_t fix){
	...
	uint32_t e, r, c;
	{
		prm = get_sr_prm_hash(sdb->indexs[idx], PRM);
		if(prm){
	...
	for(i=0;i<sdb->n_idx;i++){
		...
		for(s=0;s<2;s++){
			...
			for(j=0;j<=aux->rd_len-(klen+sdb->kmer_size);j++){
				beg = ((int)j) - ((int)len1);
				if(beg < 0) beg = 0;
				end = ((int)j) - ((int)len1) + ((int)sdb->max_rd_len);
				if(end > (int)aux->rd_len) end = aux->rd_len;
				ol = end - beg;
				if(ol > aux->max_overlap || ol < aux->min_overlap) continue;
...
uint32_t sr_align2_sdb(SR_SeqDB *sdb, SR_AlnAux *aux){
	...
	for(idx=0;idx<sdb->n_idx;idx++){
		...
		if(sdb->indexs[idx] == NULL) continue;
