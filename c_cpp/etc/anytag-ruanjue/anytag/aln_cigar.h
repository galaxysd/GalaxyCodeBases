...
static inline AlnCigarIter* init_iter_2cigars(AlnCigar *c1, int n1, AlnCigar *c2, int n2){
	AlnCigarIter* iter;
	iter = malloc(sizeof(AlnCigarIter));
	iter->c1 = c1;
	iter->c2 = c2;
	iter->n1 = n1;
	iter->n2 = n2;
	...
	iter->l2 = 0;
	iter->len   = 0;
	iter->type1 = ALN_CIGAR_TYPE_NULL;
	iter->type2 = ALN_CIGAR_TYPE_NULL;
	return iter;
}

static inline int iter_2cigars(AlnCigarIter *iter){
	if(iter->l1 == 0 && iter->m1 + 1 < iter->n1){ iter->l1 = iter->c1[++iter->m1].len; }
	if(iter->l2 == 0 && iter->m2 + 1 < iter->n2){ iter->l2 = iter->c2[++iter->m2].len; }
	if(iter->l1 == iter->l2){
		if(iter->l1 == 0) return 0;
		iter->len   = iter->l1;
		iter->type1 = iter->c1[iter->m1].type;
		iter->type2 = iter->c2[iter->m2].type;
		iter->l1 = 0;
		iter->l2 = 0;
	} else if(iter->l1 < iter->l2){
		if(iter->l1 == 0){
			iter->len   = iter->l2;
			iter->type1 = ALN_CIGAR_TYPE_NULL;
			iter->type2 = iter->c2[iter->m2].type;
...
static inline void reset_iter_2cigars(AlnCigarIter *iter, AlnCigar *c1, int n1, AlnCigar *c2, int n2){
	iter->l1 = 0;
	iter->l2 = 0;
	iter->m1 = -1;
	iter->m2 = -1;
...
static inline void free_iter_2cigars(AlnCigarIter *iter){
	free(iter);
}
...
static inline int cigars_seq2aln(char *dst, AlnCigar *c, int n, int seq_idx, char *seq){
	...
	for(i=m=k=0;i<n;i++){
		if((c[i].type >> seq_idx) & 0x01){
			...
			for(j=0;j<c[i].len;j++) dst[m++] = (i && i + 1 < n)? '-' : ' ';
		}
	}
	dst[m] = '\0';
	return m;
