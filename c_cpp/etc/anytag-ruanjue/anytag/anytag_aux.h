...
static inline void print_options(ATOptions *opt, FILE *out){
	uint32_t i;
	fprintf(out, "Options:\n");
	fprintf(out, "-g %llu\n", (unsigned long long)opt->genome_size);
	fprintf(out, "-t %d\n", opt->n_cpu);
...
static inline int parse_options(ATOptions *opt, int mode, int argc, char **argv){
	...
	opt->flags[2]     = 0;
	opt->load_aln     = NULL;
	while((c = getopt(argc, argv, "hg:t:c:X:Y:x:y:d:f:r:R:S:123i:ab:j:n:N:k:l:s:m:C:G")) != -1){
		switch(c){
			case 'h': return -1;
			...
			case 'j': opt->ol_step = atoi(optarg); break;
			...
			case 'N': opt->seed_skips[mode] |= 1U << (atoi(optarg) - 1); break;
