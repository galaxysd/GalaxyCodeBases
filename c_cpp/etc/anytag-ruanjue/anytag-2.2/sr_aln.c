...
uint32_t sr_align_long_sdb(SR_SeqDB *sdb, SR_AlnAux *aux, uint32_t lrid, uint64_t *seqs, uint64_t seqoff, uint32_t seqlen, uint32_t strand_type, uint32_t max_query){
	SR_Primer PRM;
	SR_AlnPtr *ptr;
	...
	for(j=0;j+2*sdb->kmer_size<=aux->rd_len&&j+sdb->max_rd_len<=aux->rd_len;j++){
		...
		if(max_query && n_pit >= max_query) break;
	}
	n_pit = 0;
	clear_u64list(aux->pids);
	for(i=0;i<count_sr_ptrv(aux->ptrs);i++){
