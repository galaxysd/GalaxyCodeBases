...
\t-d Result SQLite data file (_result_sv.sqlite)
...
$opt_o='__result_sv.txt' if ! $opt_o;
$opt_d='_result_sv.sqlite' if ! $opt_d;
...
my $sth = $dbh->prepare( "SELECT chrid,primary_inf,COUNT(1) FROM resv$opt_s
