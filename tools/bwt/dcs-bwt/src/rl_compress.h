...
class BitEncoder {
 public:
  ...
  void Connect(OutStreamBuffer* out) { output_.Connect(out); }
  OutStreamBuffer* Disconnect() { return output_.Disconnect(); }
 ...
 private:
  uint32 low_;
  uint32 high_;
  StreamMaster<OutStreamBuffer> output_;
...
class BitDecoder {
 public:
  ...
  void Connect(InStreamBuffer* out) { input_.Connect(out); }
  InStreamBuffer* Disconnect() { return input_.Disconnect(); }
 ...
 private:
  ...
  unsigned char ReadByte() { return input_->ReadByte(); }
};
