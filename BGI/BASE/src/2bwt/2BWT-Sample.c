...
int main() {
    ...
    unsigned long long l,r,rev_l,rev_r;
    
    //Variables for search all sa ranges functions
    unsigned long long result_l[ALPHABET_SIZE];
    unsigned long long result_r[ALPHABET_SIZE];
    unsigned long long result_rev_l[ALPHABET_SIZE];
    unsigned long long result_rev_r[ALPHABET_SIZE];
    ...
    fflush(stdout);
    Idx2BWT * idx2BWT = BWTLoad2BWT("ncbi.genome.fa.index_64.a8",".sa4");
    printf("DONE\n\n"); 
    
    // Convert the pattern into 2BWT recognised coding scheme
    BWTConvertPattern(idx2BWT,pattern,patternLength,(unsigned char*) pattern);
...
// |
    ...
    printf("SA Range being = %llu %llu (%llu)\n\n",l,r,r-l+1);
// |
// ===================================================================================
...
// |
    ...
    printf("SA Range being = %llu %llu %llu %llu (%llu)\n\n",l,r,rev_l,rev_r,r-l+1);
// |
// ===================================================================================
...
// |
    ...
    j = patternLength / 2;
    BWTSARangeInitial(idx2BWT,pattern[patternLength-1],&l,&r);
    for (i=patternLength-2;i>j-1;i--) { BWTSARangeBackward(idx2BWT,pattern[i],&l,&r); }
    
    for (i=j-1;i>=0;i--) {
        BWTAllSARangesBackward(idx2BWT,l,r,result_l,result_r);
        for (c=0;c<ALPHABET_SIZE;c++) {
            if (c==pattern[i]) continue;
            unsigned long long err_l=result_l[c];
            unsigned long long err_r=result_r[c];
            for (k=i-1;k>=0;k--) {
                if (err_l>err_r) break;
                BWTSARangeBackward(idx2BWT,pattern[k],&err_l,&err_r);
            }
            if (err_l<=err_r && k<0) {
        ...
        l=result_l[pattern[i]];
        r=result_r[pattern[i]];
    }
    
    BWTSARangeInitial(idx2BWT,pattern[0],&l,&r);
    BWTSARangeInitial(idx2BWT,pattern[0],&rev_l,&rev_r);
    for (i=1;i<j;i++) { BWTSARangeForward_Bidirection(idx2BWT,pattern[i],&l,&r,&rev_l,&rev_r); }
    for (i=j;i<patternLength;i++) {
        ...
        for (c=0;c<ALPHABET_SIZE;c++) {
            ...
            unsigned long long err_l=result_l[c];
            unsigned long long err_r=result_r[c];
            unsigned long long rev_err_l=result_rev_l[c];
            unsigned long long rev_err_r=result_rev_r[c];
            for (k=i+1;k<patternLength;k++) {
                if (err_l>err_r) break;
                BWTSARangeForward_Bidirection(idx2BWT,pattern[k],&err_l,&err_r,&rev_err_l,&rev_err_r);
            }
        ...
        l=result_l[pattern[i]];
        r=result_r[pattern[i]];
        rev_l=result_rev_l[pattern[i]];
        rev_r=result_rev_r[pattern[i]];
    }
    printf("%llu SA-indexes/occurrences were found.\n\n",saCount);
// |
// |
// ===================================================================================
...
// |
    ...
    for (i=0;i<j;i++) {
        ...
        printf("Occurrence found in sequence #%u with offset %llu\n",sequenceId,offset);
    }
// |
// |
// ===================================================================================
