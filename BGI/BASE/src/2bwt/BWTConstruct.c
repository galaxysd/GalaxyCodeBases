...
BWTInc *BWTIncCreate(MMPool *mmPool, const unsigned long long textLength, const float targetNBit,
    ...
    initializeVALLONG(bwtInc->cumulativeCountInCurrentBuild, ALPHABET_SIZE + 1, 0);
...
static void BWTIncConstruct(BWTInc *bwtInc, const unsigned long long numChar) {
    ...
    unsigned long long *relativeRank, *sortedRank, *seq;
    ...
    if (bwtInc->bwt->textLength == 0) {        // Initial build
        ...
        insertBwt = (unsigned int*) seq;
        relativeRank = (unsigned long long*)(seq + bwtInc->buildSize + 1);
    ...
    bwtInc->bwt->occValue = (unsigned int *) mergedBwt - mergedOccSizeInWord;
...
static void BWTIncSetBuildSizeAndTextAddr(BWTInc *bwtInc) {
    ...
    bwtInc->textBuffer = (unsigned char*)(bwtInc->workingMemory + (bwtInc->buildSize + CHAR_PER_WORD) * 2);
...
static void BWTIncPutPackedTextToRank(const unsigned int *packedText, unsigned long long* __restrict rank, unsigned long long* __restrict cumulativeCount, const unsigned long long numChar) {
    ...
    unsigned char ALIGN_16 temp[16]; //cover 128 bits
    ...
    mask = _mm_set1_epi32(0x0F0F0F0F); //0000 1111 0000 1111 

    //p2 = _mm_srli_epi32(p1, 4);
    //p1 = _mm_unpacklo_epi8(p1, p1);
...
static void BWTIncBuildPackedBwt(const unsigned long long *relativeRank, unsigned int* __restrict bwt, const unsigned long long numChar,
    ...
    for (i=1; i<=numChar; i++) {
        ...
        //2009-05-14 DELETE c = (previousRank > cumulativeCount[1]) + (previousRank > cumulativeCount[2]) 
        //2009-05-14 DELETE                                         + (previousRank > cumulativeCount[3]);
...
unsigned long long BWTGenerateOccValueToFileFromBwt(const char *bwtFileName, const char *occValueFileName, unsigned long long*  decodeTable) {
    ...
    if (decodeTable == NULL) {
        decodeTable = (unsigned long long*) MMUnitAllocate(DNA_OCC_CNT_TABLE_SIZE_IN_WORD * sizeof(unsigned long long));
    ...
    MMUnitFree(decodeTable, DNA_OCC_CNT_TABLE_SIZE_IN_WORD * sizeof(unsigned long long));
...
void BWTGenerateSaValue(BWT *bwt, const unsigned long long saValueFreq, unsigned int showProgressInterval) {
    ...
    if (bwt->saInterval != ALL_ONE_MASK_64) {
    ...
    #ifdef DEBUG
    initializeVAL(bwt->saValue, numberOfSaValue, ALL_ONE_MASK_64);
    #endif
    ...
    #ifdef DEBUG
    if (numberOfMatchInVAL(bwt->saValue, numberOfSaValue, ALL_ONE_MASK_64) > 0) {
...
void BWTGenerateInverseSa(BWT *bwt, const unsigned long long inverseSaFreq, unsigned long long showProgressInterval) {

    unsigned long long saValue;
    unsigned long long saIndex;
    unsigned long long numberOfInverseSa;
    unsigned long long numberOfInverseSaGenerated;
    unsigned long long progressInterval;
    unsigned long long i;
    
    if (bwt->inverseSaInterval != ALL_ONE_MASK_64) {
    ...
    #ifdef DEBUG
    initializeVAL(bwt->inverseSa, numberOfInverseSa, ALL_ONE_MASK_64);
    #endif
    ...
    while (numberOfInverseSaGenerated < numberOfInverseSa) {
        ...
        if (showProgressInterval > 0) {
            printf("Inverse SA generated : %llu\n", numberOfInverseSaGenerated);
        }
    }

    #ifdef DEBUG
    if (numberOfMatchInVAL(bwt->inverseSa, numberOfInverseSa, ALL_ONE_MASK_64) > 0) {
...
void BWTSaveInverseSa(const BWT *bwt, const char *inverseSaFileName) {
    ...
    fwrite(&bwt->inverseSa0, sizeof(unsigned long long), 1, inverseSaFile);
    fwrite(bwt->cumulativeFreq + 1, sizeof(unsigned long long), ALPHABET_SIZE, inverseSaFile);
    fwrite(&bwt->inverseSaInterval, sizeof(unsigned long long), 1, inverseSaFile);
    fwrite(bwt->inverseSa, sizeof(unsigned long long), bwt->inverseSaSizeInWord, inverseSaFile);

    fclose(inverseSaFile);

}

void BWTGenerateCachedSaIndex(const BWT *bwt, const unsigned long long numOfChar, const char *cachedSaIndexFileName) {


    // stack
    unsigned long long ALIGN_16 saIndexLeft[MAX_ARPROX_MATCH_LENGTH * ALPHABET_SIZE];
    unsigned long long ALIGN_16 saIndexRight[MAX_ARPROX_MATCH_LENGTH * ALPHABET_SIZE];
    unsigned char generatedPattern[MAX_ARPROX_MATCH_LENGTH];
    unsigned long long pos;
    unsigned long long i;
    unsigned long long numOfPattern;
    unsigned long long saRangeIndex, saIndex;
    ...
    unsigned long long *cachedSaIndex;
    ...
    fwrite(&bwt->inverseSa0, sizeof(unsigned long long), 1, cachedSaIndexFile);
    fwrite(bwt->cumulativeFreq + 1, sizeof(unsigned long long), ALPHABET_SIZE, cachedSaIndexFile);
    fwrite(&numOfChar, sizeof(unsigned int), 1, cachedSaIndexFile);

    numOfPattern = 1 << (numOfChar * 2);    // 4^numOfChar
    cachedSaIndex = (unsigned long long*) MMUnitAllocate((numOfPattern+1) * sizeof(unsigned long long));
    ...
    saRangeIndex = (unsigned long long)-1;
    ...
    QSort(saIndexLeft + 1, numOfChar - 1, sizeof(unsigned long long), QSortUnsignedIntOrder);
    ...
    fwrite(cachedSaIndex, sizeof(unsigned long long), numOfPattern, cachedSaIndexFile);
    MMUnitFree(cachedSaIndex, (numOfPattern+1) * sizeof(unsigned long long));

    fclose(cachedSaIndexFile);

}

void BWTGenerateSaBitmap(const BWT *bwt, const unsigned long long numOfChar, const char *saBitmapFileName) {

    // stack
    unsigned long long saIndexLeft[MAX_ARPROX_MATCH_LENGTH + 1];
    unsigned long long saIndexRight[MAX_ARPROX_MATCH_LENGTH];
    unsigned char generatedPattern[MAX_ARPROX_MATCH_LENGTH];
    unsigned long long pos;
    unsigned long long i, c;
    unsigned LONG numOfPattern;
    unsigned long long numOfPatternExistInText = 0;
    ...
    fwrite(&bwt->inverseSa0, sizeof(unsigned long long), 1, saBitmapFile);
    fwrite(bwt->cumulativeFreq + 1, sizeof(unsigned long long), ALPHABET_SIZE, saBitmapFile);
    fwrite(&numOfChar, sizeof(unsigned long long), 1, saBitmapFile);

    bitmapSize = 1 << (numOfChar * 2 - 5);    // 4^numOfChar
    saBitMap = (unsigned long long*) MMUnitAllocate(bitmapSize * sizeof(unsigned long long));
    ...
    fwrite(saBitMap, sizeof(unsigned long long), bitmapSize, saBitmapFile);
    MMUnitFree(saBitMap, bitmapSize * sizeof(unsigned long long));

    numOfPattern = (unsigned LONG)1 << (numOfChar * 2);    // 4^numOfChar

    printf("%llu out of %llu length %llu patterns exist in text.\n", numOfPatternExistInText, numOfPattern, numOfChar);

    fclose(saBitmapFile);

}

void BWTGenerateCompressedSaBitmap(const BWT *bwt, const unsigned long long numOfChar, const char *saBitmapFileName) {
    ...
    unsigned long long lastBitValue, bitValue;
    unsigned long long groupSize;

    unsigned long long tempIntegerStream[1025];
    unsigned long long numOfInteger;
    unsigned long long numOfHighBit, numOfLowBit;

    unsigned long long tempOneStream[1025];
    unsigned long long tempZeroStream[1025];
    unsigned long long numOfOne, numOfZero;
    ...
    while (pos < numOfChar) {
        ...
        if (saIndexLeft[pos] <= saIndexRight[pos]) {
            saBitMap[saBitmapIndex >> 5] |= FIRST_BIT_MASK >> ((unsigned long long)saBitmapIndex & 0x1F);
            numOfPatternExistInText++;
        }
    }
...
void BWTCountPattern(const BWT *bwt, const unsigned long long numOfChar) {
...
/*
void BWTGenerateSaRangeTable(const BWT *bwt, const unsigned long long numOfChar, const char *PackedDNAFileName, const char *saRangeFileName, const char *startSaIndexFileName) {

    // stack
    unsigned long long saIndexLeft[MAX_ARPROX_MATCH_LENGTH + 1];
    unsigned long long saIndexRight[MAX_ARPROX_MATCH_LENGTH];
    unsigned char generatedPattern[MAX_ARPROX_MATCH_LENGTH];
    unsigned long long endOfTextPattern[MAX_ARPROX_MATCH_LENGTH];
    unsigned long long endOfTextChar;
    unsigned long long pos;
    unsigned long long i, c;
    unsigned long long numOfPattern;
    unsigned long long saRangeIndex, lastSaRangeIndex;
    unsigned long long mask[16] = { 0xFFFFFFFC, 0xFFFFFFF3, 0xFFFFFFCF, 0xFFFFFF3F,
                     0xFFFFFCFF, 0xFFFFF3FF, 0xFFFFCFFF, 0xFFFF3FFF,
                     0xFFFCFFFF, 0xFFF3FFFF, 0xFFCFFFFF, 0xFF3FFFFF,
                     0xFCFFFFFF, 0xF3FFFFFF, 0xCFFFFFFF, 0x3FFFFFFF };
...
void BWTPrintAndVerifyOccValue(const BWT *bwt, FILE *output) {
    ...
    unsigned long long occValue[ALPHABET_SIZE];
...
void BWTPrintAndVerifySaValue(const BWT *bwt, FILE *output) {
    ...
    unsigned long long i;
    unsigned long long saValue;
    unsigned long long *printedValue;

    printedValue = (unsigned long long*) malloc(sizeof(unsigned long long) * (bwt->textLength + 1));
    initializeVALLONG(printedValue, bwt->textLength + 1, FALSE);
    
    for (i=0; i<=bwt->textLength; i++) {
        saValue = BWTSaValue(bwt, i);
        if (output != NULL) {
            fprintf(output, "%llu ", saValue);
        }
    ...
    if (numberOfMatchInVALLONG(printedValue, bwt->textLength + 1, FALSE) > 0) {
