...
for (my $c=1; $c<=22; $c++) {
	...
	for (my $j=1; $j<$Window_num; $j++) {
		...
		if ($gap_ratio < 0.5 && ($j*$Window_size > 2000000) && ($j*$Window_size < $ChrLen{$chr} - 2000000) ) {
...
mkdir("chr_accumu_Mb_vs_cM_curves_win$Window_size");
for (my $i=1; $i<=23; $i++) {
	my $chr = "chr$i";
	$chr =~ s/23/X/g;
	my $lst_file = "chr_accumu_Mb_vs_cM_curves_win$Window_size/$chr.accumulative.Mb.cM.lst";
...
open OUT, ">correlation_between_genetic_maps_win$Window_size.tab" || die "fail";
