...
my $blat_client_options = ' -out=psl -minIdentity=95 -maxIntron=5 -noHead';
my $target_genome = "hg18.2bit";
my $virus_genome = "virus.2bit";
my $boundary;
my $flank_region_sz=500;
my $dir_2bit = "/";
...
my $optionOK = GetOptions(
	...
	's|virus_genome=s'	=> \$virus_genome,
	'boundary=s'	=> \$boundary,
	'flankregion=s'	=> \$flank_region_sz,
...
my $mapper = Mapper->new(
	-PRG => $blat_exe, #join(' ', ($blat_client_exe, $blat_server, $blat_port)),
	-OPTIONS => $blat_client_options,
	-BIT2_DIR => $target_genome,##$dir_2bit,
	-MAX_SCORE_DIFF => $max_score_diff,
	-MAX_NUM_HITS => $max_num_hits,
);
my $mapperV = Mapper->new(
	-PRG => $blat_exe, #join(' ', ($blat_client_exe, $blat_server, $blat_port)),
	-OPTIONS => $blat_client_options,
	-BIT2_DIR => $virus_genome,##$dir_2bit,
	-MAX_SCORE_DIFF => $max_score_diff,
	-MAX_NUM_HITS => $max_num_hits,
);
...
my (@boundarys, @Putative_CHRs, @Putative_lowbound, @Putative_upbound, @Evidences, @low_conf);

if($boundary){
    @boundarys = `awk \'{print \$4"\\t"\$5"\\t"\$7"\\t"\$8"\\t"\$9}\' $boundary`;
    foreach (@boundarys){
        my @SD = split /\t/;
        ...
        push @Putative_CHRs,     $SD_chr;
        push @Putative_lowbound, $bound[0];
        push @Putative_upbound,  $bound[1];
        push @Evidences, $SD[4];
    }

    if (-e $output_file.'2'){
        my $output_file2 = $output_file.'2';
        `rm $output_file2`;
    }
}
...
if($range) {
	...
	foreach my $chr (keys %sclip) {
                ...
                if($boundary){
                    if ($chr ne 'chrVirus'){
                        for (0..(scalar(@Putative_CHRs)-1)){
                           my $tmp_chr = $Putative_CHRs[$_];
                           $tmp_chr = substr($tmp_chr,3) if ($chr !~ /^chr/ && lc($tmp_chr) =~ /^chr/);
                           if ($chr eq $tmp_chr){
                              $g_flag = 1;
                              last;
                           }
		...
		foreach my $sv (@tmpSV) {
		    push @final_SV, $sv if($sv && ! is_dup_SV(\@final_SV, $sv));
                ...
                if($boundary){
                    if (scalar(@tmpSV)==0 && scalar(@low_conf)>0){
                        open OUT, '>>', $output_file.'2';
                        foreach (@low_conf) {
                            print OUT "$g_flag\t$_\n";
                        }
                        close OUT;
...
open my $OUTF, ">$output_file-filtered" or croak "can't open $output_file-filtered:$OS_ERROR";
foreach my $SV (@final_SV) {
	if($SV->filter) {
		...
		print $OUT "\n";
                ...
                print $OUTF $SV->to_string ;
                if($RNASeq) {
                        print $OUTF "\t", join("\t", @{$SV->get_genes});
                }
                print $OUTF "\n";
                ...
                if($boundary){
                    if ($chr ne 'chrVirus'){
                        ...
                        for (0..(scalar(@Putative_CHRs)-1)){
                           ...
                           $tmp_chr = substr($tmp_chr,3) if ($chr !~ /^chr/ && lc($tmp_chr) =~ /^chr/);
                           if ($chr eq $tmp_chr &&
                               $pos>=$Putative_lowbound[$_]-$flank_region_sz &&
                               $pos<=$Putative_upbound[$_]+$flank_region_sz &&
                               ($Evidences[$_]+scalar(@s_reads))/$cover_s>=0.05){
                              $flag = 1;
                              last;
                           }
                        }
                        ...
                        next if (scalar(@s_reads)*1000<$cover_s);
                    }
                }

		print STDERR join("\t", ($chr, $pos, $clip == LEFT_CLIP ? "-" : "+", scalar @s_reads)), "\n";

                if ($boundary && $chr ne 'chrVirus'){
                    push @low_conf, join("\t", ($chr, $pos, $clip == LEFT_CLIP ? "-" : "+", scalar @s_reads, $cover_s));
                }
...
sub detect_SV {
	...
	my $mapping;
        ...
        if ($chr eq 'chrVirus'){
            $mapping = $mapper->run( -QUERY => $contig_file );
        }else{
            $mapping = $mapperV->run( -QUERY => $contig_file );
        }
...
sub check_sclip {
	...
	if($RNASeq) { #let's do a wild guess!
		...
		$orig = join("", ($orig, $pos-$read_len < 1 ? 1:$pos-$read_len, "-", $pos +  $read_len));
	}
