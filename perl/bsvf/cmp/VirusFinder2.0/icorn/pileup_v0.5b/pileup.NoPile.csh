...
set prog=/nfs/team81/jit/bin/pileup_v0.5/ssaha_pileup # set ssaha_pileupProgramPath
set ssaha2Path=/nfs/team81/jit/bin/pileup_v0.5/ssaha2/ssaha2-2.3_x86_64
...
if($pairend == 1) then
	$ssaha2Path -rtype $rtype -kmer $kmer -score $score -seeds $seeds -skip $skip -diff $diff -memory $memory -cut $cut -pair $pairedLow,$pairedHigh -output cigar $ref $reads > tmp.cigar1.$$
else
	$ssaha2Path -rtype $rtype -kmer $kmer -score $score -seeds $seeds -skip $skip -diff $diff -memory $memory -cut $cut -output cigar $ref $reads > tmp.cigar1.$$
endif;
