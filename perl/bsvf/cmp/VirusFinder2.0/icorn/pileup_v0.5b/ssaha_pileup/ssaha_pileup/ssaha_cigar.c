...
//    system("ps aux | grep eulerSNP; date");
    ...
    if(argc < 2)
    {
      printf("Usage: %s <cigar_line_file> <output_cigar_file>\n",argv[0]);
      exit(1);
    }
...
/* ====================================================  */
{
     int i;
//     char **rdname;
     char *ptr,line[2000],rdname[100],rdnamep[100],RC;
     FILE *namef,*namef2;
     int name_flag=0,num_align=0;
     int hit_score = 0; 
       ...
       { 
         ...
         for(ptr=strtok(line2," ");ptr!=NULL;ptr=strtok((char *)NULL," "),i++)
         {
            if(i==0)
            {
              ...
              memset(base,'\0',500);
              strcat(base,ptr);
//              strcpy(cigarhead,ptr);
	      len = strlen(ptr);
	      hit_score = 0;
//	      printf("%d %s",len,line3);
	      if(len<8)
	        add_flag = 1;
	      else
	      {
		...
		hit_score = atoi(ptr);
	      ...
	      if(add_flag)
	      {
            ...
            else if((i==6)&&(!uniq_flag))
            {
              memset(base,'\0',500);
              strcat(base,ptr);
              hit1 = atoi(ptr);
            }
            else if((i==7)&&(!uniq_flag))
            {
              memset(base,'\0',500);
              strcat(base,ptr);
              hit2 = atoi(ptr);
            }
            else if((i==9)&&(!uniq_flag))
            {
              memset(base,'\0',500);
              strcat(base,ptr);
              m_score = atoi(ptr);
            ...
            {
              strcpy(rdnamep,rdname);
	      if(uniq_flag)
	      {
   	        if((!name_flag)&&(hit_score>0))
		{
	          fprintf(namef2,"%s",line3);
                  num_align++;
		}
	      }
	      else
