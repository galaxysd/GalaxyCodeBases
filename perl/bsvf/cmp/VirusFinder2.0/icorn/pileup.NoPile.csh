...
# 24/06/2008
#
# modified for the set of variable and other stuff
# 03.07.09
# tdo@sanger.ac.uk

set prog=$PILEUP_HOME/ssaha_pileup # set ssaha_pileupProgramPath
set ssaha2Path=$PILEUP_HOME/ssaha2
#tdo/bin/pileup_v0.4/ssaha2 # set ssaha2ProgramPath
...
if($pairend == 1) then
	$ssaha2Path/ssaha2-2.3_$arch -rtype $rtype -kmer $kmer -score $score -seeds $seeds -skip $skip -diff $diff -memory $memory -cut $cut -pair $pairedLow,$pairedHigh -output cigar $ref $reads > tmp.cigar1.$$
else
	$ssaha2Path/ssaha2-2.3_$arch -rtype $rtype -kmer $kmer -score $score -seeds $seeds -skip $skip -diff $diff -memory $memory -cut $cut -output cigar $ref $reads > tmp.cigar1.$$
endif;
...
#	sed 's/\.F /.p1k /' tmp.cigar2.$$ | sed 's/\.R /.q1k /' > tmp.$result.cigar3.$$
   ...
   echo "paried"
	if($paired == 1) then 
		...
		if($rtype == "solexa") then
			echo "solexa"
			$prog/ssaha_pileup/ssaha_clean -insert $insertSize -std $std tmp.cigar2c.$$ tmp.$result.cigar3.$$
			\rm tmp.cigar2c.$$
		else 
