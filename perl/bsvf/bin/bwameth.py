...
def bwa_mem(fa, mfq, extra_args, prefix='bwa-meth', threads=1, rg=None,
    ...
    # cmd = "|bwa mem -T 40 -B 2 -L 10 -CM " # samtools 1.5 cannot read sam with appended FASTA/FASTQ comment like `1:N:0:CATCATAA`.
    cmd = "|bwa mem -T 40 -B 2 -L 6 -MY "
...
def as_bam(pfile, fa, prefix, calmd=False, set_as_failed=None):
    ...
    """
    view = "samtools view -bS - | samtools sort -n -m 2415919104 - -T {bam} -o "
    if calmd:
        cmds = [
            view + "{bam}.tmp",
