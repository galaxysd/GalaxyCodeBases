...
my ($chrid,$pos,$pos2,$scaffold,$scaffold2,$start,$end,$qres,$newpos,$newpos2,@last);
while (<IN>) {
# ChrNew1	121900	T T T T W W T T T T T T T T T T W W T T T T T T T - T T T T T T T T T T
	($chrid,$pos,$pos2,@last)=split /\t/;
	...
	$stho->execute($chrid,$pos2);
	$qres = $stho->fetchall_arrayref;
	if ($#$qres == -1) {
		warn "No info. for $chrid:$pos, skipped.\n";
		next;
	} elsif ($#$qres != 0) { warn "More than 1 hit, using first.\n" }
	($scaffold2,$start,$end)=@{$$qres[0]};
	$newpos2=1+$pos2-$start;
	if ($scaffold ne $scaffold2) {
		print OUT "[$scaffold,$scaffold2] ";
		warn "Diff scaffold happens. ";
	}
	print OUT join("\t",$scaffold,$newpos,$newpos2,@last);
}
close OUT;
close IN;
#END
