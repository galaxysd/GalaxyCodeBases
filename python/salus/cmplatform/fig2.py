...
def main(thisID) -> None:
    ...
    sc.pl.pca(adata, color='Platform', ax=ax)
    plt.savefig(f"2C_PCA_{nfoDict['sid']}.pdf", transparent=True, dpi=300, metadata={'Title': 'PCA', 'Subject': f"{nfoDict['sub']} Data", 'Author': 'HU Xuesong'})
    fig, ax = plt.subplots(figsize=(6, 6), dpi=300)
    ax.set_aspect(1)
    plt.tight_layout(pad=1)
    plt.title(f"UMAP - {nfoDict['sub']}")
    sc.pl.umap(adata, color='Platform', ax=ax)
    plt.savefig(f"2C_UMAP_{nfoDict['sid']}.pdf", transparent=True, dpi=300, metadata={'Title': 'UMAP', 'Subject': f"{nfoDict['sub']} Data", 'Author': 'HU Xuesong'})
    fig, ax = plt.subplots(figsize=(6, 6), dpi=300)
    ax.set_aspect(1)
    plt.tight_layout(pad=1)
    plt.title(f"t-SNE - {nfoDict['sub']}")
    sc.pl.tsne(adata, color='Platform', ax=ax)
    plt.savefig(f"2C_tsne_{nfoDict['sid']}.pdf", transparent=True, dpi=300, metadata={'Title': 't-SNE', 'Subject': f"{nfoDict['sub']} Data", 'Author': 'HU Xuesong'})
    fig, ax = plt.subplots(figsize=(6, 6), dpi=300)
    ax.set_aspect(1)
    plt.tight_layout(pad=1)
    plt.title(f"ForceAtlas2 - {nfoDict['sub']}")
    sc.pl.draw_graph(adata, color='Platform', ax=ax)
    plt.savefig(f"2C_ForceAtlas2_{nfoDict['sid']}.pdf", transparent=True, dpi=300, metadata={'Title': 'ForceAtlas2', 'Subject': f"{nfoDict['sub']} Data", 'Author': 'HU Xuesong'})
