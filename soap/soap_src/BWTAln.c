...
static inline int POSCMP(const void *a, const void *b){
 ...
 int BWTGapMatching(const unsigned char *convertedKey, const BWTOPT *bo, const int strain, BWT *bwt, BWT *rev_bwt, LOOKUPTABLE *lookup, LOOKUPTABLE *rev_lookup, HITTABLE *hits){
	/*	
	if (convertedKey == NULL) return 0;
	LOOKUPTABLE lookupTable, rev_lookupTable;
	lookupTable.tableSize = lookup->tableSize;
	...
	unsigned int cutoff       = bo->cutoff;
	unsigned int keyLength    = bo->alnLen;
	unsigned int MaxGap       = bo->gap_len;
//	fprintf(stderr, "GAPPING: allowed gap: %d\n", MaxGap);
	unsigned int GapFb        = bo->gap_fb;
	...
	const unsigned int max_mm   = bo->max_mm;
	const unsigned int *pacRef  = bo->pacRef;
	unsigned int info = 0;
	unsigned int occ_pos  = 0;
	char *seq    =  strain?bo->rc:(bo->fw+alnLen);

	unsigned int l, r, tmpl, tmpr;
	l = r = tmpl = tmpr = 0;
	unsigned int rev_l, rev_r;
	unsigned int i;
	int inc=0;
	int gapOpen , gapExt, gapEnd;
	gapOpen = keyLength;
	gapExt = gapEnd = 0;
	unsigned long long *OCCRangeStack1 = (unsigned long long *)malloc(sizeof(unsigned long long)*(keyLength-lookupTable.tableSize));
	unsigned long long *pr1 = OCCRangeStack1;
	unsigned long long *OCCRangeStack2 = (unsigned long long *)malloc(sizeof(unsigned long long)*(keyLength-lookupTable.tableSize));
	unsigned long long *pr2 = OCCRangeStack2;
	...
	l = packedPattern ? lookupTable.table[packedPattern-1]+1:1;
	r = lookupTable.table[packedPattern];
//	fprintf(stderr, "l : r %u\t%u\n", l, r);
	if (l <= r) {
		gapEnd = keyLength - i;
		unsigned char c;
//		fprintf(stderr, "here\n");
		for (i = keyLength - lookupTable.tableSize; i > 0 && l <= r; --i, --gapEnd) {
//			fprintf(stderr, "Arr: %u\t%u\n", l, r);
			*pr1++ = (((unsigned long long)l<<32)|((unsigned long long)r));
			c = convertedKey[i-1] & 0x3;
			l = bwt->cumulativeFreq[c] + BWTOccValue(bwt, l, c) + 1;
			r = bwt->cumulativeFreq[c] + BWTOccValue(bwt, r + 1, c);
		}
		/*
		if (i<=5) {
			unsigned int info = ( );
			pr1--;
			hitcount += OCCProcess(((*pr1)>>32) & 0xffffffff, (*pr1) & 0xffffffff, bo, info, hits);
	...
	for (i = 0; i < rev_lookupTable.tableSize; ++i) {
		l_packedPattern<<=2;
		l_packedPattern |= (convertedKey[i]&0x3);
	}
	r_packedPattern = l_packedPattern;
	//If the look-up tables are of different size
	l_packedPattern <<= (lookupTable.tableSize-rev_lookupTable.tableSize)*2;
	...
	rev_l = rev_packedPattern ? rev_lookupTable.table[rev_packedPattern-1]+1 : 1;
	rev_r = rev_lookupTable.table[rev_packedPattern];
//	fprintf(stderr, "O: %d\tE: %d\n", gapOpen, gapEnd);
	if (l <= r) {
		// forward search with BWT until the end of forward depth section
		for (i = rev_lookupTable.tableSize; i <= gapEnd && l <= r; ++i) {
//			fprintf(stderr, "Arr2: %u\t%u\n", l, r);
			*pr2++ = (((unsigned long long)l<<32)|((unsigned long long)r));
			unsigned char c = convertedKey[i] & 0x3 ;
			BWTAllOccValue(rev_bwt,rev_l,occCount_start);
			BWTAllOccValue(rev_bwt,rev_r + 1,occCount_end);
			rev_l = rev_bwt->cumulativeFreq[c] + occCount_start[c] + 1;
			rev_r = rev_bwt->cumulativeFreq[c] + occCount_end[c];
...
//	fprintf(stderr, "O: %d\tE: %d\n", gapOpen, gapEnd);
	if (gapEnd - gapOpen <= MaxGap) {
//		fprintf(stderr, "Here, O: %d\tE: %d\n", gapOpen, gapEnd);
		pr1--; pr2--;
		do{
//			fprintf(stderr, "in cyc\n");
			int c1 = ((*pr1)&0xffffffff) - (((*pr1)>>32)&0xffffffff) +1;
			int c2 = ((*pr2)&0xffffffff) - (((*pr2)>>32)&0xffffffff) +1;
			unsigned int *arr1 = malloc(sizeof(unsigned int) * c1);
			unsigned int *arr2 = malloc(sizeof(unsigned int) * c2);
			unsigned int *q = arr1;
			unsigned int *p = arr2;
			l = ((*pr1)>>32)&0xFFFFFFFF; r = (*pr1)&0xFFFFFFFF;
//			fprintf(stderr, "arr: %u\t%u\n", l, r);
			GenOCCArr(arr1);
			l = ((*pr2)>>32)&0xFFFFFFFF; r = (*pr2)&0xFFFFFFFF;
//			fprintf(stderr, "arr: %u\t%u\n", l, r);
			GenOCCArr(arr2);
			qsort(arr1, c1, sizeof(unsigned int), POSCMP);
			qsort(arr2, c2, sizeof(unsigned int), POSCMP);
//			fprintf(stderr, "n: %d\t%d\n", c1, c2);
			q = arr1; p = arr2;
			while (1) {
				if (q == arr1 + c1 || p == arr2 + c2) break;
//				fprintf(stderr, "%u\t%u\n", *p, *q);
				while(*p + gapOpen > *q && q != arr1+c1) q++;
//				fprintf(stderr, "%u\t%u\n", *p, *q);
				while( q != arr1+c1 && *p+gapOpen+MaxGap < *q && p != arr2+c2 ) p++;
//				fprintf(stderr, "%u\t%u\n", *p, *q);
				if (q == arr1 + c1 || p == arr2 + c2) break;
//				fprintf(stderr, "gap size: %d\n",*q-*p-gapOpen);
				if (*q-*p-gapOpen < MaxGap) {
					/*
					fprintf(stderr, "GAP: %u\t%u\n", *p, *q);
					fprintf(stderr, "ext: %d\n", gapEnd-gapOpen);
					//
					if(gapEnd - gapOpen > 1)		// del
						info = 4<<25|((strain&0x1)<<24)|((gapOpen&0xff)<<12)|(gapEnd-gapOpen);
					else		// insertion 
						info = 3<<25|((strain&0x1)<<24)|((gapOpen&0xff)<<12)|((*q-*p-gapOpen)&0xff);
					occ_pos = *p;
					HitInc(n);
				}
				p++; q++;
				...
				if (inc > 0){ 
					hits->n = n; 
					free(arr1); free(arr2); 
					free(OCCRangeStack2); free(OCCRangeStack1); 
					return inc;
				}
			}
			pr1--; pr2--;
			...
			/*
			   ...
			   +1    insertion
			 /
//	fprintf(stderr, "To be finish\n");
	free(OCCRangeStack2); free(OCCRangeStack1);
	return inc;
	// Gap in Backward lookup keyPattern
	//DELETION
	//*/
}
