...
=pod
To do:
-m cache lines into $hash{"$fileid_$offset"} so that just need to read 1 times.

If sync read over NFS will benifit from NFS cache ...
-t time to wait for sync read.
	1. `date > $opt_o/.Signal/$opt_c.ready`
	2. wait -t seconds for $opt_o/.Signal/$opt_c.GO
	3. if -e $opt_o/.Signal/$opt_c.GO {begin reading} else {goto 2}
	4. after -t seconds, `touch $opt_o/.Signal/$opt_c.GO` & begin reading
	5. when reading begin, `date >> $opt_o/.Signal/$opt_c.ready`
	In this way, $opt_o/.Signal/$opt_c.GO can be touch manually, or just wait for -t.
-f chrorder file way to wait for sync. Just wait all $opt_c.ready and then GO.
-p -f use UDP port -p to communicate(pingpong within this perl) instead of files over NFS. 
...
our $help=<<EOH;
\t-i SOAP result list, in format: /^[PS]E\\t/path/to/soap.file\\n\$/
...
print STDERR "\n";
#if ($opt_c) {print "Only parse Chromosome [$opt_c].\n";}
# else {print "Parsing All Chromosomes.\n";}
...
my (%FILES);
open SAMPLE,'<',$opt_i or die "Error opening $opt_i: $!\n";
while (<SAMPLE>) {
	chomp;
	my ($pe,$file)=split /\t/;
	push @{$FILES{$pe}},$file;
...
for my $file (@{$FILES{SE}}) {
	++$fileid;	# will be the No. of files
	$the_time = [gettimeofday];
